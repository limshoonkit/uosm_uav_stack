...
{
    PolyTrajOptimizer::PolyTrajOptimizer(const rclcpp::Node::SharedPtr &node)
    {
        node_ = node;

        using uosm::util::getParam;

        RCLCPP_INFO(node_->get_logger(), "Polynomial Optimizer Parameters:");
        cps_num_per_piece_ = getParam<int>(node, "optimization.constraint_points_per_piece", 5, " * Constraint pts/piece: ");
        wei_obs_ = getParam<double>(node, "optimization.weight_obstacle", 10000.0, " * Weight obstacle: ");
        wei_obs_soft_ = getParam<double>(node, "optimization.weight_obstacle_soft", 5000.0, " * Weight obstacle soft: ");
        wei_swarm_ = getParam<double>(node, "optimization.weight_swarm", 10000.0, " * Weight swarm: ");
        wei_feas_ = getParam<double>(node, "optimization.weight_feasibility", 10000.0, " * Weight feasibility: ");
        wei_sqrvar_ = getParam<double>(node, "optimization.weight_sqrvariance", 10000.0, " * Weight sqrvariance: ");
        wei_time_ = getParam<double>(node, "optimization.weight_time", 10.0, " * Weight time: ");
        obs_clearance_ = getParam<double>(node, "optimization.obstacle_clearance", 0.1, " * Obstacle clearance: ");
        obs_clearance_soft_ = getParam<double>(node, "optimization.obstacle_clearance_soft", 0.5, " * Obstacle clearance soft: ");
        swarm_clearance_ = getParam<double>(node, "optimization.swarm_clearance", 0.15, " * Swarm clearance: ");
        max_vel_ = getParam<double>(node, "optimization.max_vel", 2.0, " * Max vel: ");
        max_acc_ = getParam<double>(node, "optimization.max_acc", 6.0, " * Max acc: ");
        max_jer_ = getParam<double>(node, "optimization.max_jer", 4.0, " * Max jer: ");
                                               ...
                                               double &final_cost)
    {
        ...
        // Preparation 1: Some misc params
        constexpr int MAX_RESTART = 3;
        constexpr int MAX_REBOUND = 20;
        ...
        // Preparation 2: Trajectory related params
        ...
        // Preparation 3: LBFGS related params
        ...
        do
        {
                ...
                result == lbfgs::LBFGS_STOP)
            {
                ...
                if (!flag_swarm_too_close)
                {
                    if (finelyCheckAndSetConstraintPoints(segments_nouse, jerkOpt_, false) == CHK_RET::OBS_FREE)
                    {
                        ...
                        flag_success = true;
                        // RCLCPP_INFO(node_->get_logger(),
                        //             "iter=%d, time(ms)=%.3f, total_t(ms)=%.3f, cost=%.3f",
                        //             iter_num_, time_ms, total_time_ms, final_cost);
                    }
                    else
                    {
                        // A non-blank return value means collision to obstacles
                        flag_still_unsafe = true;
                        restart_nums++;
                        // RCLCPP_INFO(node_->get_logger(),
                        //             "iter=%d, time(ms)=%.3f, fine check collided, keep optimizing",
                        //             iter_num_, time_ms);
                    }
                }
                else
                {
                    // RCLCPP_INFO(node_->get_logger(),
                    //             "Swarm clearance not satisfied, keep optimizing. iter=%d, time(ms)=%.3f, wei_swarm_mod_=%f",
            ...
            else if (result == lbfgs::LBFGSERR_CANCELED)
            {
                ...
                RCLCPP_DEBUG(node_->get_logger(),
                             "iter=%d, time(ms)=%.3f, rebound",
                             iter_num_, time_ms);
            }
            else
            {
                // RCLCPP_INFO(node_->get_logger(), "iter=%d, time(ms)=%.3f, error", iter_num_, time_ms);
                RCLCPP_WARN_STREAM(node_->get_logger(), "Solver error. Return = " << result << ", " << lbfgs::lbfgs_strerror(result) << ". Skip this planning.");
            }

        } while ((flag_still_unsafe && restart_nums < MAX_RESTART) ||
                 (flag_force_return && force_stop_type_ == STOP_FOR_REBOUND && rebound_times <= MAX_REBOUND));

        // auto traj_post_optim = jerkOpt_.getTraj();
        // int PN = traj_post_optim.getPieceNum();
        // for (int i = 0; i < PN; ++i)
        // {
        //     const auto &cMat = traj_post_optim.getPiece(i).getCoeffMat();
        //     RCLCPP_INFO(node_->get_logger(), "traj_post_optim[%d] = \n%f, %f, %f, %f, %f, %f\n %f, %f, %f, %f, %f, %f\n %f, %f, %f, %f, %f, %f", i,
    ...
    bool PolyTrajOptimizer::computePointsToCheck(poly_traj::Trajectory &traj, int id_cps_end, PtsChk_t &pts_check)
    {
        pts_check.clear();
        pts_check.resize(id_cps_end);
        const double RES = grid_map_resolution_, RES_2 = RES / 2;
        ...
        double t = 0.0, t_step = std::min(RES / max_vel_, durations.minCoeff() / std::max(cps_num_per_piece_, 1) / 1.5);
        ...
        while (true)
        {
            ...
            const double next_t_stp = t_seg_start(id_piece_curr) + durations(id_piece_curr) / cps_num_per_piece_ * ((id_cps_curr + 1) - cps_num_per_piece_ * id_piece_curr);
            if (t >= next_t_stp)
            {
                if (id_cps_curr + 1 >= cps_num_per_piece_ * (id_piece_curr + 1))
                {
                    ++id_piece_curr;
                }
        ...
        const bool flag_first_init /*= true*/)
    {
        ...
        Eigen::MatrixXd init_points = pt_data.getInitConstraintPoints(cps_num_per_piece_);
        ...
        for (size_t i = 0; i < segment_ids.size(); ++i)
        {
            ...
            AStarResult ret = a_star_->search(grid_map_resolution_, in, out);
            if (ret == AStarResult::SUCCESS)
            {
                a_star_paths.push_back(a_star_->get_path());
            }
            else if (ret == AStarResult::SEARCH_ERROR && i + 1 < segment_ids.size()) // connect the next segment
            {
                segment_ids[i].second = segment_ids[i + 1].second;
                segment_ids.erase(segment_ids.begin() + i + 1);
                --i;
                RCLCPP_WARN(node_->get_logger(), "A corner case 2, I have never test it.");
            }
            else
            {
                RCLCPP_ERROR(node_->get_logger(), "A-star error, force return!");
                return CHK_RET::ERR;
            }
        }
        ...
        for (size_t i = 0; i < segment_ids.size(); i++)
        {
            ...
            for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
            {
                ...
                double val = (a_star_paths[i][Astar_id] - init_points.col(j)).dot(ctrl_pts_law), init_val = val;
                while (true)
                {
                ...
                if (got_intersection_id >= 0)
                {
                    ...
                    if (length > 1e-5)
                    {
                        ...
                        for (double a = length; a >= 0.0; a -= grid_map_resolution_)
                        {
                            bool occ = grid_map_->getInflateOccupancy((a / length) * intersection_point + (1 - a / length) * init_points.col(j));

                            if (occ || a < grid_map_resolution_)
                            {
                                if (occ)
                                    a += grid_map_resolution_;
    ...
    bool PolyTrajOptimizer::roughlyCheckConstraintPoints(void)
    {
        ...
        for (int i = 1; i <= i_end; ++i)
        {
            ...
            if (occ)
            {
                ...
                {
                    if ((cps_.points.col(i) - cps_.base_point[i][k]).dot(cps_.direction[i][k]) < 1 * grid_map_resolution_) // current point is outside all the collision_points.
                    {
                        occ = false;
                        break;
                    }
        ...
        if (flag_new_obs_valid)
        {
            ...
            for (size_t i = 0; i < segment_ids.size(); ++i)
            {
                ...
                AStarResult ret = a_star_->search(/*(in-out).norm()/10+0.05*/ grid_map_resolution_, in, out);
                if (ret == AStarResult::SUCCESS)
                {
                    a_star_paths.push_back(a_star_->get_path());
                }
    ...
    /* multi-topo support */
    std::vector<ConstraintPoints> PolyTrajOptimizer::distinctiveTrajs(std::vector<std::pair<int, int>> segments)
    {
        ...
        int seg_upbound = std::min<int>(static_cast<int>(segments.size()), static_cast<int>(std::floor(std::log(MAX_TRAJS) / std::log(VARIS))));
        std::vector<ConstraintPoints> control_pts_buf;
        control_pts_buf.reserve(MAX_TRAJS);
        const double RESOLUTION = grid_map_resolution_;
        ...
        for (int i = 0; i < seg_upbound; i++)
        {
            ...
            exit_multi_loop2:;
                ...
                for (int j = occ_start_id; j <= occ_end_id; j++)
                {
                    ...
                    if (RichInfoSegs[i].first.base_point[j].size() != 1)
                    {
                        ...
                        oss << "RichInfoSegs[" << i << "].first.base_point[" << j << "].size()=" << RichInfoSegs[i].first.base_point[j].size();
                        ...
                        oss << "cps_" << std::endl;
                        oss << " clearance=" << obs_clearance_ << " cps.size=" << cps_.cp_size << std::endl;
                        for (int temp_i = 0; temp_i < cps_.cp_size; temp_i++)
                        {
                            if (cps_.base_point[temp_i].size() > 1 && cps_.base_point[temp_i].size() < 1000)
                            {
                                ...
                                oss << "######" << cps_.points.col(temp_i).transpose() << std::endl;
                                for (size_t temp_j = 0; temp_j < cps_.base_point[temp_i].size(); temp_j++)
                                    oss << "      " << cps_.base_point[temp_i][temp_j].transpose() << " @ " << cps_.direction[temp_i][temp_j].transpose() << std::endl;
                            }
                        }
                        RCLCPP_ERROR(node_->get_logger(), "%s", oss.str().c_str());
                    ...
                    else
                    {
                        ...
                        std::ostringstream oss;
                        oss << "base_point=" << RichInfoSegs[i].first.base_point[j][0].transpose()
                            << " control point=" << RichInfoSegs[i].first.points.col(j).transpose();
            ...
            else
            {
                ...
                else
                {
                    ...
                    std::ostringstream oss;
                    oss << "base_point=" << RichInfoSegs[i].first.base_point[0][0].transpose()
                        << " control point=" << RichInfoSegs[i].first.points.col(0).transpose();
                    RCLCPP_WARN(node_->get_logger(), "%s", oss.str().c_str());
        ...
        for (int i = 0; i < max_traj_nums; i++)
        {
            ...
            while (/*seg_id < RichInfoSegs.size() ||*/ cp_id < cps_.cp_size)
            {
                ...
                {
                    RCLCPP_ERROR(node_->get_logger(), "Should not happen!!!!, cp_id=%d, seg_id=%d, segments.front().first=%d, segments.back().second=%d, segments[seg_id].first=%d, segments[seg_id].second=%d",
    ...
    double PolyTrajOptimizer::costFunctionCallback(void *func_data, const double *x, double *grad, const int n)
    {
        ...
        std::fill(opt->min_ellip_dist2_.begin(), opt->min_ellip_dist2_.end(), std::numeric_limits<double>::max());
        ...
        opt->addPVAJGradCost2CT(gradT, obs_swarm_feas_qvar_costs, opt->cps_num_per_piece_); // Time int cost
        ...
        opt->jerkOpt_.getGrad2TP(gradT, gradP); // Gradient propagation
    ...
    {
        grid_map_ = map;
        grid_map_resolution_ = grid_map_->getResolution();
        a_star_ = std::make_shared<AStar>();
        a_star_->init_grid_map(grid_map_, Eigen::Vector3i(100, 100, 100));
    }
    ...
    void PolyTrajOptimizer::setDynamicLimits(double max_vel, double max_acc)
    {
        max_vel_ = max_vel;
        max_acc_ = max_acc;
    }

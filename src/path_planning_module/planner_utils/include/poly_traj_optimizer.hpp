#ifndef _POLY_TRAJ_OPTIMIZER_HPP_
#define _POLY_TRAJ_OPTIMIZER_HPP_
...
namespace ego_planner
{
  ...
  public:
    ...
    void segment(ConstraintPoints &buf, const int start, const int end)
    {
      ...
      {
        std::cout << "Error: Wrong segment index! start=" << start << ", end=" << end << std::endl;
        return;
      }
  ...
  private:
    ...
    int drone_id_;
    int cps_num_per_piece_;               // number of distinctive constraint points each piece
  ...
  public:
    PolyTrajOptimizer(const rclcpp::Node::SharedPtr &node);
    ...
    inline int get_cps_num_per_piece_(void) { return cps_num_per_piece_; }

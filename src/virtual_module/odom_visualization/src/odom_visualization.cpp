...
void cmd_callback(const mavros_msgs::msg::PositionTarget cmd)
{
...
int main(int argc, char **argv)
{
    ...
    mesh_resource = getParam<std::string>(node, "mesh_resource", "package://odom_visualization/meshes/hummingbird.mesh");
    color_r       = getParam<double>(node, "color.r",            1.0);
    color_g       = getParam<double>(node, "color.g",            0.0);
    color_b       = getParam<double>(node, "color.b",            0.0);
    color_a       = getParam<double>(node, "color.a",            1.0);
    origin        = getParam<bool>(node, "origin",              false);
    scale         = getParam<double>(node, "robot_scale",        2.0);
    _frame_id     = getParam<std::string>(node, "frame_id",    "map");

    cross_config  = getParam<bool>(node, "cross_config",        false);
    tf45          = getParam<bool>(node, "tf45",                false);
    cov_scale     = getParam<double>(node, "covariance_scale", 100.0);
    cov_pos       = getParam<bool>(node, "covariance_position", false);
    cov_vel       = getParam<bool>(node, "covariance_velocity", false);
    cov_color     = getParam<bool>(node, "covariance_color",    false);
    _drone_id     = getParam<int>(node, "drone_id",               -1);


    // 发布者和订阅者
    auto qos = rclcpp::SensorDataQoS();
    auto sub_odom = node->create_subscription<nav_msgs::msg::Odometry>(
        "odom", qos, odom_callback);
    auto sub_cmd = node->create_subscription<mavros_msgs::msg::PositionTarget>(

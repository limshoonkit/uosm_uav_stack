...
int main(int argc, char** argv) {
  ...
  sensing_horizon = getParam<double>(node, "sensing_horizon",   0.0);
  sensing_rate    = getParam<double>(node, "sensing_rate",      0.0);
  estimation_rate = getParam<double>(node, "estimation_rate",   0.0);
  _x_size         = getParam<double>(node, "map.x_size",       0.0);
  _y_size         = getParam<double>(node, "map.y_size",       0.0);
  _z_size         = getParam<double>(node, "map.z_size",       0.0);
  ...
  odom_sub = node->create_subscription<nav_msgs::msg::Odometry>(
      "odometry", qos, rcvOdometryCallbck);
